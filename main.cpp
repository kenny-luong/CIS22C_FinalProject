...
bool LoadBusinesses (BST<Type> &tree, Hash<Type> &hash) {
    ...
    while (!businessData.eof()) {
      getline(businessData, tempName);
      getline(businessData, tempAddress);
      businessData >> numOfRatings;
      businessData >> sumOfRatings;
      getline(businessData, garbage);
  ...
  while (true) {
    ...
    switch (mainMenu) {
      case 1: {
        ...
        tree.insertBST(*newBusiness, newBusiness->getName());
        hash.add(newBusiness->getName(), *newBusiness);
        break;
      }
      case 2: {
        string name;
